...
- **Emby 反向代理**：代理 Emby 请求，拦截 PlaybackInfo 强制 DirectPlay，支持 302 重定向（默认）和流式代理两种模式可切换，302 模式客户端直连 115 CDN 消除中转带宽消耗
...
播放时代理返回 302 跳转让客户端直连 115 CDN，媒体流量不经过代理服务器；也可切换为流式代理模式由服务端中转。跨域问题通过注入 crossOrigin 拦截脚本解决。
